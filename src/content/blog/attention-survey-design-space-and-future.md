...
description: "Surveying the efficient transformer landscape from the ground up — building one 8-token running example with a full 8×8 attention matrix, then showing how each family of efficient transformer sparsifies, factorizes, or approximates that same matrix; deriving the quadratic cost of vanilla attention, walking through fixed patterns (Image Transformer, Sparse Transformer, Longformer, BigBird), combination patterns (Axial), learnable patterns (Reformer, Routing, Sinkhorn), neural memory (Set Transformer, ETC, Compressive), low-rank factorization (Linformer, Synthesizer), kernel methods (Performer, Linear Transformer), segment recurrence (Transformer-XL), downsampling (Perceiver, Funnel, Nyströmformer), and conditional computation (Switch, GShard); deriving a single unified weight-function view that collapses every method into the common equation $o_i = \\sum_j w_{ij} v_j$ where each method picks a different $w$; ranking all variants on the impossible triangle of training parallelism, low-cost inference, and strong quality; and ending with the three credible futures of attention — RetNet's hybrid retention, Mamba's selective state spaces, and DeepSeek-V2's multi-head latent attention — each derived from the same running example to show exactly where they sit in the design space — all derived step by step with concrete examples."
date: 2026-04-10
...
The previous blogs in this series built the efficient transformer landscape one piece at a time. We started from scaled dot-product attention, asked what could be modified, and then modified everything: the number of KV heads (GQA and the KV Bottleneck blogs), the attention pattern (Sliding Window, Sparse Factorization, DeepSeek Sparse Attention), the gating (Gated Attention, Gated DeltaNet), the residual path (Hyper-Connections), the kernel (Linear Attention, Kernel Zoo), the recurrence structure (RetNet), the latent space (DeepSeek-V2 MLA), and even the starting point itself (Mamba, which begins from state space models and works backward toward attention).

Each of those blogs took one idea and derived it end-to-end. This blog does something different. It places every variant on a single grid, computes what each does to the *same* attention matrix, and asks: what is the shape of the design space? Where has the field converged? And given that convergence, what are the plausible futures of attention?

The core reference for the survey portion is Tay et al. (2022), "Efficient Transformers: A Survey" (arXiv:2009.06732v3). For the three futures we revisit Sun et al. (2023) "Retentive Network: A Successor to Transformer for Large Language Models," Gu and Dao (2023) / Dao and Gu (2024) on Mamba and Mamba-2, and DeepSeek-AI (2024) on DeepSeek-V2.

The goal is a single unified view. Every efficient attention mechanism replaces the dense $N \times N$ attention matrix $A$ with something cheaper. Either it sparsifies $A$ (fills it with zeros), factorizes $A$ (writes it as a product of smaller matrices), approximates $A$ (replaces the softmax kernel with a finite feature map), recurses $A$ (turns the horizontal scan into a vertical state update), or pools $A$ (shrinks one of its dimensions). We will derive each of these moves from the same numbers.
