...
This post is not part of the attention-architectures series — it is a systems paper about inference deployment, and the attention-side facts enter only as inputs. We do assume familiarity with what a KVCache is and why it dominates long-context inference cost; if that is new, [The KV Bottleneck Explained](/blog/attention-kv-bottleneck) is the right background. We will derive the entire throughput model of PrfaaS-PD from scratch on one tiny two-cluster deployment. Every equation — KV throughput, cluster egress bandwidth, the three producer-consumer stages, the balance conditions on threshold $t$ and prefill/decode ratio $N_p/N_d$ — will be instantiated in concrete numbers. At the end we will reproduce the paper's headline result (selective offloading beats both homogeneous and naive heterogeneous baselines) on our toy setup.
...
**Prefill-decode (PD) disaggregation** separates LLM inference into two phases that run on different hardware: prefill, which consumes the whole prompt and emits the KVCache, and decode, which reads that KVCache and emits tokens one by one. The motivation is that the two phases have different bottlenecks. Prefill is compute-bound (a single batched matmul over the entire prompt). Decode is memory-bandwidth-bound (reading the KV tensors once per new token). Pooling both on the same GPU starves one of them.

The catch is that once prefill and decode live on different machines, the KVCache has to *move*. In a conventional Transformer, that movement is massive. Consider one prefill instance in our deployment. If requests arrive back to back and each produces $S_{\mathrm{kv}}(l)$ megabytes of KV state in $T_{\mathrm{prefill}}(l)$ seconds, then the instance emits bytes at a sustained rate that we call its **KV throughput**.
