...
Linear attention replaces the softmax similarity $\exp(q^\top k / \sqrt{d_k})$ with an inner product $\phi(q)^\top \phi(k)$ for some feature map $\phi : \mathbb{R}^{d_k} \to \mathbb{R}^{d_\phi}$. Once that swap is made, the matrix-product associativity $(\phi(Q)\phi(K)^\top) V = \phi(Q)(\phi(K)^\top V)$ collapses the $O(n^2)$ attention matrix into a constant-size state $S = \phi(K)^\top V \in \mathbb{R}^{d_\phi \times d_v}$, and the layer becomes an RNN that runs in linear time. The framework is fully general — every linear-attention method is just a different choice of $\phi$.

So the design question is not *whether* to linearize but *which* $\phi$ to pick. And the choice of $\phi$ controls two things at once. It sets the **capacity** of the memory: a state of dimension $d_\phi$ can store at most $d_\phi$ orthogonal key-value pairs before retrieval starts to interfere. And it sets the **approximation fidelity**: how closely $\phi(q)^\top \phi(k)$ tracks the actual softmax similarity that the original Transformer was trained to use. Make $\phi$ low-dimensional and the layer is cheap but the memory saturates fast. Make it high-dimensional and the memory is roomier but the constants on the cost grow. Make it carefully designed (random features, orthogonal projections, positive maps) and the same dimension can target either capacity or fidelity — but not both equally well.
